...
| 観察対象 | 述語 | 行動 |
|---|---|---|
| 論理要素と物理構造 | 物理構造が同じ業務意味と不変条件を保存する | 型、技術列、派生表、冗長化、materialized view、partitionを選べる。各写像に一次データ、同期、再構築、撤去方法を書く。論理設計は事実から導ける情報（いまの状態、件数、合計）を持たないので、性能や制約のためにそれを保存するのはこの物理設計の判断であり、保存する理由と、元の事実との整合の保ち方を必ず書く |
