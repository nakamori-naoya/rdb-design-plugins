...
読み終えると、論理データモデル資料の意味を保ったまま、上流制約をRDB固有の実現へ写し、根拠、検証状態、見直し条件を持つ`rdb-physical-design`本文を作って検査できる。

## 入力

- `grounded_physical_inputs`: 事実、合意済み決定、仮説、未確認、観測結果へ分類済みの入力
- `logical_document_path`: 論理データモデル資料の絶対path
- `database_product` / `database_version`: 対象RDB製品と一つに定まる版
- 改訂時だけ`existing_physical_document_path`: 既存の物理設計資料の絶対path

入力に含まれる要求・負荷・品質・基盤・証拠は、それぞれの資料を変更せず設計根拠として使う。

## 判断基準

| 観察対象 | 述語 | 行動 |
|---|---|---|
| 論理要素と物理構造 | 物理構造が同じ業務意味と不変条件を保存する | 型、技術列、派生表、冗長化、materialized view、partitionを選べる。各写像に一次データ、同期、再構築、撤去方法を書く。論理設計はデータを加工して得る情報（件数、合計、平均）を持たないので、性能や制約のためにそれを保存するのはこの物理設計の判断であり、保存する理由と、元の事実との整合の保ち方を必ず書く |
...
| transaction | 論理データモデルの「並行実行で必要な保証」に、同時に進む操作と許してはいけない結果が具体化されている | 操作ごとに、対象版の分離レベル、制約、lock、競合検出と、transaction の中で直列化の失敗やデッドロックを何回まで繰り返すかを決める。分離レベルと繰り返しの回数の持ち主はこの物理設計であり、実装はここで決めた指定を渡すだけにする。名称だけでは完了にしない |
...
1. `python3 scripts/physical_design.py fingerprint --model-file <論理データモデル資料の絶対path>`を実行し、論理構造の指紋を得る。終了code 0以外なら本文を作らない。
...
5. `write-doc`の`rdb-physical-design`型が定める節構成で完成本文を作る。検査が読むラベルの行、表の見出し行、小見出しの先頭の名前は、`write-doc`の公開契約の「検査が読む目印」に従う。BDDや論理テーブル定義は複製せず、論理データモデル資料と指紋を参照する。
6. 完成本文を標準入力で`python3 scripts/physical_design.py check --model-file <論理データモデル資料の絶対path> --product <製品> --version <版>`へ渡す。終了code 0は構造契約への適合、1は`problem`診断、2は入力を読めないことを示す。1なら本文を直して再検査し、2または解消不能なら止まる。

## 停止条件

**止まる。** 論理データモデル資料を読めない、対象製品・版が空、toolが失敗する、物理実現に論理意味または不変条件の変更が必要な場合である。差し戻す論理変更、その根拠、物理側で代替できない理由を返す。
